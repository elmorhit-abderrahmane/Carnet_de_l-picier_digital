    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for (var i = 0; i < FormersList.length; i++){
               ...
               <td>${FormersList[i]["name"]}</td>
               <td>${FormersList[i]["stock"]}</td>
               <td>${FormersList[i]["image"]}</td>
               <td>${FormersList[i]["Quantity"]}</td>
...
$("body").on("click",".FormersList .update-btn",function() {
    ...
    $(".edit-form .name").val(FormersList[index]["name"]);
    $(".edit-form .stock").val(FormersList[index]["stock"]);
    $(".edit-form .image").val(FormersList[index]["image"]);
    $(".edit-form .Quantity").val(FormersList[index]["Quantity"]);
    $(".edit-form .sid").val(FormersList[index]["id"]);
  });
  
  // save-Former-database
  $(".save-former").click(function() {
    var Fname = $(".edit-form .name").val();
    var Lname = $(".edit-form .stock").val();
    var image = $(".edit-form .image").val();
    var Quantity = $(".edit-form .Quantity").val();
    var Id = $(".edit-form .sid").val();
    console.log(Fname,Lname,image,Quantity,Id)
